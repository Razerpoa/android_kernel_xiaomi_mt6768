...
static void __input_release_device(struct input_handle *handle) {
  ...
  grabber = rcu_dereference_protected(dev->grab, lockdep_is_held(&dev->mutex));
  if (grabber == handle) {
    rcu_assign_pointer(dev->grab, NULL);
    /* Make sure input_pass_event() notices that grab is gone */
    synchronize_rcu();

    list_for_each_entry(handle, &dev->h_list,
                        d_node) if (handle->open && handle->handler->start)
...
static void input_disconnect_device(struct input_dev *dev) {
  ...
  list_for_each_entry(handle, &dev->h_list, d_node) handle->open = 0;
...
int input_set_keycode(struct input_dev *dev,
  ...
  if (old_keycode > KEY_MAX) {
    ...
    struct input_value vals[] = {{EV_KEY, old_keycode, 0}, input_value_sync};
...
static int input_bits_to_string(char *buf, int buf_size, unsigned long bits,
  ...
  if (in_compat_syscall()) {
    ...
    if (dword || !skip_empty || len)
      len += snprintf(buf + len, max(buf_size - len, 0), "%x", dword);
  } else {
    if (bits || !skip_empty)
...
static void *input_devices_seq_next(struct seq_file *seq, void *v,
                                    loff_t *pos) {
...
static void input_seq_print_bitmap(struct seq_file *seq, const char *name,
  ...
  for (i = BITS_TO_LONGS(max) - 1; i >= 0; i--) {
    if (input_bits_to_string(buf, sizeof(buf), bitmap[i], skip_empty)) {
      skip_empty = false;
...
static int input_print_modalias(char *buf, int size, struct input_dev *id,
  ...
  len = snprintf(buf, max(size, 0), "input:b%04Xv%04Xp%04Xe%04X-",
                 id->id.bustype, id->id.vendor, id->id.product, id->id.version);

  len += input_print_modalias_bits(buf + len, size - len, 'e', id->evbit, 0,
                                   EV_MAX);
  len += input_print_modalias_bits(buf + len, size - len, 'k', id->keybit,
                                   KEY_MIN_INTERESTING, KEY_MAX);
  len += input_print_modalias_bits(buf + len, size - len, 'r', id->relbit, 0,
                                   REL_MAX);
  len += input_print_modalias_bits(buf + len, size - len, 'a', id->absbit, 0,
                                   ABS_MAX);
  len += input_print_modalias_bits(buf + len, size - len, 'm', id->mscbit, 0,
                                   MSC_MAX);
  len += input_print_modalias_bits(buf + len, size - len, 'l', id->ledbit, 0,
                                   LED_MAX);
  len += input_print_modalias_bits(buf + len, size - len, 's', id->sndbit, 0,
                                   SND_MAX);
  len += input_print_modalias_bits(buf + len, size - len, 'f', id->ffbit, 0,
                                   FF_MAX);
  len += input_print_modalias_bits(buf + len, size - len, 'w', id->swbit, 0,
                                   SW_MAX);
...
static int input_print_bitmap(char *buf, int buf_size, unsigned long *bitmap,
  ...
  for (i = BITS_TO_LONGS(max) - 1; i >= 0; i--) {
    len += input_bits_to_string(buf + len, max(buf_size - len, 0), bitmap[i],
                                skip_empty);
    if (len) {
      skip_empty = false;
      if (i > 0)
...
#define INPUT_DEV_CAP_ATTR(ev, bm)                                             \
  static ssize_t input_dev_show_cap_##bm(                                      \
      struct device *dev, struct device_attribute *attr, char *buf) {          \
    struct input_dev *input_dev = to_input_dev(dev);                           \
    int len = input_print_bitmap(buf, PAGE_SIZE, input_dev->bm##bit, ev##_MAX, \
                                 true);                                        \
    return min_t(int, len, PAGE_SIZE);                                         \
  }                                                                            \
...
static const struct attribute_group *input_dev_attr_groups[] = {
    &input_dev_attr_group, &input_dev_id_attr_group, &input_dev_caps_attr_group,
    NULL};
...
static int input_add_uevent_bm_var(struct kobj_uevent_env *env,
  ...
  len = input_print_bitmap(&env->buf[env->buflen - 1],
                           sizeof(env->buf) - env->buflen, bitmap, max, false);
...
static int input_dev_uevent(struct device *device,
                            struct kobj_uevent_env *env) {
  struct input_dev *dev = to_input_dev(device);

  INPUT_ADD_HOTPLUG_VAR("PRODUCT=%x/%x/%x/%x", dev->id.bustype, dev->id.vendor,
...
struct input_dev *devm_input_allocate_device(struct device *dev) {
  ...
  devres = devres_alloc(devm_input_device_release, sizeof(*devres), GFP_KERNEL);
  if (!devres)
    return NULL;
...
void input_free_device(struct input_dev *dev) {
  if (dev) {
    if (dev->devres_managed)
      WARN_ON(devres_destroy(dev->dev.parent, devm_input_device_release,
...
static unsigned int input_estimate_events_per_packet(struct input_dev *dev) {
  ...
  if (test_bit(EV_ABS, dev->evbit))
    for_each_set_bit(i, dev->absbit, ABS_CNT) events +=
...
int input_register_device(struct input_dev *dev) {
  ...
  if (dev->devres_managed) {
    devres =
        devres_alloc(devm_input_device_unregister, sizeof(*devres), GFP_KERNEL);
    if (!devres)
      return -ENOMEM;
...
void input_unregister_device(struct input_dev *dev) {
  if (dev->devres_managed) {
    WARN_ON(devres_destroy(dev->dev.parent, devm_input_device_unregister,
                           devm_input_device_match, dev));
...
int input_get_new_minor(int legacy_base, unsigned int legacy_num,
   ...
   */
  if (legacy_base >= 0) {
    int minor = ida_simple_get(&input_ida, legacy_base,
...
static int __init input_init(void) {
  ...
  err = register_chrdev_region(MKDEV(INPUT_MAJOR, 0), INPUT_MAX_CHAR_DEVICES,
                               "input");
  if (err) {
