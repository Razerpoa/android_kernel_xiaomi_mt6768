...
struct mount *__lookup_mnt(struct vfsmount *mnt, struct dentry *dentry) {
  ...
  hlist_for_each_entry_rcu(p, head,
                           mnt_hash) if (&p->mnt_parent->mnt == mnt &&
                                         p->mnt_mountpoint == dentry) return p;
  return NULL;
}
...
static void commit_tree(struct mount *mnt) {
  ...
  list_for_each_entry(m, &head, mnt_list) m->mnt_ns = n;
...
static void delayed_mntput(struct work_struct *unused) {
  ...
  llist_for_each_entry_safe(m, t, node, mnt_llist) cleanup_mnt(m);
}
...
static struct vfsmount *fs_set_subtype(struct vfsmount *mnt,
                                       const char *fstype) {
  int err;
...
long do_mount(const char *dev_name, const char __user *dir_name,
  ...
  if ((flags & MS_REMOUNT) && ((flags & (MS_NOATIME | MS_NODIRATIME |
  ...
  if (flags & MS_REMOUNT)
    ...
    retval = do_new_mount(&path, type_page, sb_flags, mnt_flags, dev_name,
                          data_page);
dput_out:
  path_put(&path);
  return retval;
...
void __init mnt_init(void) {
  ...
  mount_hashtable = alloc_large_system_hash(
      "Mount-cache", sizeof(struct hlist_head), mhash_entries, 19, HASH_ZERO,
      &m_hash_shift, &m_hash_mask, 0, 0);
  mountpoint_hashtable = alloc_large_system_hash(
      "Mountpoint-cache", sizeof(struct hlist_head), mphash_entries, 19,
      HASH_ZERO, &mp_hash_shift, &mp_hash_mask, 0, 0);
...
bool our_mnt(struct vfsmount *mnt) { return check_mnt(real_mount(mnt)); }
